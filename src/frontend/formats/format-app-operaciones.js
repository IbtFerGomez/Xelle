...
const AppCom = {
    ...
    init: function () {
        ...
        console.log("Iniciando App Operaciones V2.3 para:", docId);
    ...
    Universal: {
        ...
        setupPrintHandler: function () {
            const getSelectDisplayValue = (sel) => {
                if (!sel) return '';

                if (sel.multiple) {
                    const values = Array.from(sel.selectedOptions || [])
                        .map(opt => String(opt.text || opt.label || opt.value || '').trim())
                        .filter(Boolean);
                    return values.join(', ');
                }

                const selected = sel.selectedOptions?.[0] || sel.options?.[sel.selectedIndex] || null;
                ...
                const matched = Array.from(sel.options || []).find(opt => String(opt.value || '').trim() === rawValue);
                const matchedText = String(matched?.text || matched?.label || '').trim();
                return matchedText || rawValue;
            };

            const ensurePrintSpan = (sel) => {
            ...
            const syncSelectPrintValue = (sel) => {
                const span = ensurePrintSpan(sel);
                if (!span) return;
                const displayValue = getSelectDisplayValue(sel);
                span.textContent = displayValue;
                sel.dataset.printValue = displayValue;
            };

            // Mantiene actualizado el texto imprimible incluso en filas agregadas dinámicamente.
            document.addEventListener('change', (event) => {
                const sel = event.target;
                if (sel && sel.tagName === 'SELECT') {
                    syncSelectPrintValue(sel);
                }
            });
            ...
            window.addEventListener('afterprint', () => {
                document.querySelectorAll('select').forEach(sel => {
                    if (!sel.dataset.printValue) {
                        sel.dataset.printValue = getSelectDisplayValue(sel);
                    }
                });
            });
        },
